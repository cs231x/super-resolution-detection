...
parser.add_argument('--train_dataset', type=str, default='DIV2K_train_LR_bicubic_X2')
...
def train(epoch):
    ...
    for iteration, batch in enumerate(training_data_loader, 1):
        inputX, target = Variable(batch[0]), Variable(batch[1])
        if cuda:
            inputX = inputX.cuda(gpus_list[0])
            target = target.cuda(gpus_list[0])

        optimizer.zero_grad()
        t0 = time.time()
        loss = criterion(model(inputX), target)
        t1 = time.time()
